...
%regexp_string = '\[t\](........)\[\\t\]';
%regexp_string = '[S]C....';
regexp_string = '\"[A-Z]+[\d]+[A-Z\d]+/"';

% Read list of tests from the source url
%list_url = 'http://anasimtiaz.com/edfx-toolbox-files/list_of_tests.txt';
list_url = [annotations_url 'list_of_tests.txt'];
edfx_webpage_source = webread(annotations_url);
...
for i=5:10
    ...
    this_test = test_names{i}(2:end-2)
